...
echo -e ""
echo -e "* [1] Installer un serveur Paper 1.16.4 (Latest build: paper-1.16.4-416.jar)"
echo -e "* [2] Récupérer une base de serveur Paper 1.16.4"
echo -e "* [3] Paramétrer un serveur Paper 1.16.4"
...
case $input in
	...
	1)
		clear
		echo -e "${GREEN}Installation de Paper 1.16.4 ...${NC}"
		...
		# Installation of Paper 1.16.4 Build 416
		echo -e "${GREEN}Téléchargement de Paper 1.16.4 Build 416 dans '$install_folder' ...${NC}"
		cd $install_folder

		sudo rm -r *

		wget -O paper-1.16.4-416.jar https://api.papermc.io/v2/projects/paper/versions/1.16.4/builds/416/downloads/paper-1.16.4-416.jar
		...
		if [ "$java_version" -eq 16 ]; then
			...
			echo -e "java -Xms512M -Xmx${ram_amount} -jar paper-1.16.4-416.jar nogui" >> start.sh
		else [ "$java_version" -gt 16 ]
			echo -e "${GREEN}Création du fichier de démarrage.${NC}"
			echo -e "java -Xms512M -Xmx${ram_amount} -DPaper.IgnoreJavaVersion=true -jar paper-1.16.4-416.jar nogui" >> start.sh
		fi
		...
		echo -e "${YELLOW}Avertissement ! N'installez pas deux fois Paper 1.16.4 dans le même dossier, cela pourrait engendrer quelques erreurs.${NC}"
		echo -e ""
		echo -e "${LIGHT_BLUE}Merci d'avoir utilisé le script d'installation de serveur Paper 1.16.4 de FireTryx !${NC}"
		echo -e ""
	...
	2)
		...
		# Get the Paper 1.16.4 base
		download_url="https://media.githubusercontent.com/media/FireTryx/Firetryx_Scripts/refs/heads/development/Paper/Base_serveurs/1.16.4/1.16.4.zip?download=true"

		# Télécharger le fichier ZIP généré par DownGit
		output_file="server_base-firetryx_script-1.16.4.zip"
